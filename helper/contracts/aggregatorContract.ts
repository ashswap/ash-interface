...
class AggregatorContract extends Contract<typeof aggregatorAbi> {
    ...
    async aggregate(tokenPayments: TokenPayment[], steps: AggregatorStep[], limits: {token: string, amount: BigNumber}[]) {
        let interaction = this.contract.methods.aggregate([steps, ...limits]);
        interaction.withMultiESDTNFTTransfer(tokenPayments, new Address(await getAddress()));
