...
const useFBAllClaimRewards = (trackStatus = false) => {
    ...
    const fbClaimAllRewards = useRecoilCallback(({snapshot, set}) => async (farmAddress: string) => {
        const fbAccountFarm = await snapshot.getPromise(fbAccountFarmSelector(farmAddress));
        if(!fbAccountFarm) return;
        const farm = new Address(farmAddress);
        sendTransactions({
            transactions: await Promise.all(fbAccountFarm.rewards.map(r => ContractManager.getFarmBribeContract(ASHSWAP_CONFIG.dappContract.farmBribe).claimReward(farm, r.tokenId))),
