...
import TokensSelectorForChart, { TokenOptionChart } from "../components/TokensSelectorForChart";
...
function AprByPoolsChart() {
    ...
    const displayChartData: IAPRByPoolData[] = useMemo(() => {
        ...
        chartData.map(val => {
            ...
            const w = timeUnit === "W" ? 
            moment.unix(val.timestamp).day(1).hour(0).minute(0).second(0).millisecond(0).unix()
            : moment.unix(val.timestamp).date(1).hour(0).minute(0).second(0).millisecond(0).unix();
            if(wMap[w]){
                wMap[w].push(val);
            }else{
                wMap[w] = [val];
        ...
        const sum = Object.keys(wMap).map(k => {
            return wMap[+k].reduce((total: IAPRByPoolData, record) => {
                data?.map((d, index) => {
                    total[`token${index}`]=(total[`token${index}`] || 0) + record[`token${index}`]
                })
                return total;
            }, {timestamp: +k});
        });
        const avg = sum.map(val => {
            const sRecord: IAPRByPoolData = {timestamp: val.timestamp};
            data?.map((d, index) => {
                sRecord[`token${index}`] = +(val[`token${index}`] / wMap[val.timestamp].length).toFixed(2);
            });
            return sRecord;
    ...
    const ticks = useMemo(() => {
        ...
        displayChartData.map(({timestamp}) => {
            if(timeUnit === "D") {
                ...
                temp.add(timeUnit === "M" ? time.date(1).unix() : time.day(1).unix());
            }
        });
    ...
    const tickFormatter = useCallback((val, index: number) => {
        ...
        return timeUnit === "D" ? time.format('DD/MM/yyyy') : timeUnit === "M" ? months[time.month()] : 'week ' + time.week();
    }, [timeUnit]);
    ...
    return (
        ...
        >
                        ...
                        {data?.map(({address}, index) => {
                            return (
                                <Line
                                    ...
                                    type="linear"
                                    dataKey={`token${index}`}
                                    stroke={themeColors[pools.findIndex(p => p.address === address)]}
                                    dot={false}
                                />
                            );
                        })}
            ...
            <div className="flex space-x-2 text-xs mt-4">
                <TokensSelectorForChart label={selectedPools.size !== 0 ? `${selectedPools.size} pools selected` : "Select pools to view"}>
                    {pools.map((p, i) => {
                        return <TokenOptionChart key={p.address} pool={p} checked={selectedPools.has(p.address)} color={themeColors[i]} label={p.tokens.map(t => t.name).join('-')} onChange={(val) => onSelectPool(val, p.address)}/>
                    })}
