...
function PoolPriceRatioAreaChart({
    pool,
    timeUnit,
    type,
}: {
    ...
    type: "ratio12" | "ratio21"
}) {
    const { data } = useSWR<[number, number][]>(
        pool.address
            ? `${ASHSWAP_CONFIG.ashApiBaseUrl}/pool/${pool.address}/graph-statistic?type=${type}`
            : null,
        fetcher
    );
...
export default PoolPriceRatioAreaChart;
