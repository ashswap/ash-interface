...
function DAOCard({ detail, proposal }: DAOCardProps) {
    ...
    const {status, minQuorumPct, minSupportPct, supportPct, quorumPct, startVoteTS, endVoteTS, yesVote, noVote, canExecuteTS, canExecute} = useDAOProposalComputedState(proposal);
    const startVoteTSFmt = useMemo(() => {
        return moment.unix(startVoteTS).format("HH:mm [(UTC]Z[)] DD, MMM, YYYY");
    }, [startVoteTS]);
    const canExecuteTSFmt = useMemo(() => {
        return moment.unix(canExecuteTS).format("HH:mm [(UTC]Z[)] DD, MMM, YYYY"); 
    }, [canExecuteTS]);
    return (
        <div className="relative px-6 sm:px-12 py-5 sm:py-9">
            <DAOCardBg
                status={status}
                clipClassName="clip-corner-4 clip-corner-br"
            />
            <div className="h-full relative flex flex-col justify-between">
                <div>
                        ...
                        {status === "approved" && (
                            <div className="relative px-3 sm:px-4 py-0.5 sm:py-1">
                                ...
                                <div className="relative font-bold text-xs sm:text-base text-white capitalize">
                                    {canExecute ? <>Ready for <span className="text-stake-green-500 underline">Execute</span></> : "On Queue"}
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <p className="mb-4 font-bold text-xs text-stake-gray-500 line-clamp-2">
                        {meta.description}
                    </p>
                    <Link href={`/stake/gov/dao/detail/?proposalID=${proposal.proposal_id}`}>
                        <span className="font-bold text-sm text-white underline">
                            Read more
                        </span>
                    </Link>
                </div>
                ...
                >
                    ...
                    <div className="mt-12 space-y-4">
                        <div className="flex items-center">
                            ...
                            <div className="font-bold text-sm text-stake-green-500">
                                <TextAmt
                                    number={supportPct}
                                    options={{ isIntegerAuto: true }}
                                />
                        ...
                        <div className="flex items-center">
                            ...
                            <div className="font-bold text-sm text-ash-purple-500">
                                <TextAmt
                                    number={quorumPct}
                                    options={{ isIntegerAuto: true }}
                                />
                    ...
                    <div className="mt-12">
                        {status === "active" && <Countdown timestamp={endVoteTS} small />}
                        {status === "pending" && <div>
                            <div className="mb-2 font-bold text-lg text-white">This proposal can be voted on</div>
                            <div className="font-bold text-sm text-stake-gray-500">{startVoteTSFmt}</div>
                            </div>}
                        {status === "approved" && <div>
                            <div className="mb-2 font-bold text-lg text-white">This proposal can be execute when</div>
                            <div className="font-bold text-sm text-stake-gray-500">{canExecuteTSFmt}</div>
                            </div>}
                    </div>
                </Transition>
            </div>
