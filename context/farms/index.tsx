...
const FarmsProvider = ({ children }: any) => {
    const [poolRecords, setPoolRecords] = useState<FarmRecord[]>([]);
    ...
    const poolToDisplay = useMemo(() => {
        let result: FarmRecord[] = [...poolRecords].filter(p => stakedOnly ? !!p.stakedData : true);
        ...
        return result;
    }, [poolRecords, deboundKeyword, sortOption, stakedOnly]);

    return (
        <FarmsContext.Provider
            value={{
                ...initState,
                farmRecords: poolRecords,
